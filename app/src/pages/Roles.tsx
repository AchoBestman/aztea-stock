  ...
  return (
    <div className="space-y-6 animate-slide-up select-none p-8 max-w-6xl mx-auto">
                ...
                roles.map(role => (
                  <tr key={role.id} className="border-b border-border/50 hover:bg-accent/20 transition-colors font-medium">
                    <td className="py-4 px-6 font-bold text-foreground flex items-center gap-2">
                      <Shield className="w-4 h-4 text-primary dark:text-blue-400 shrink-0" />
                      <span>{role.name}</span>
                      {role.is_system && (
      ...
      {isPermModalOpen && selectedRole && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-[200] overflow-y-auto animate-scale-in">
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-card border border-border w-full max-w-3xl rounded-3xl shadow-2xl flex flex-col overflow-hidden relative">
              <div className="p-6 border-b border-border flex items-center justify-between">
                <h3 className="font-extrabold text-base text-foreground">
                  Permissions : <span className="text-blue-600 dark:text-blue-400">{selectedRole.name}</span>
                </h3>
              </div>
                        ...
                        return (
                          <div 
                            ...
                            className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                              isSelected 
                                ? 'bg-blue-500/5 border-blue-500 dark:border-blue-400 shadow-sm' 
                                : 'bg-background border-border dark:border-blue-400 hover:bg-muted/50'
                            }`}
                          >
              ...
              <div className="p-6 border-t border-border flex items-center justify-end gap-3 bg-muted/10">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-primary dark:bg-blue-600 text-primary-foreground rounded-xl text-xs font-bold shadow-sm hover:bg-opacity-95 cursor-pointer transition-opacity"
                >
                  Sauvegarder
                </button>
              </div>
